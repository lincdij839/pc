...
pub const Parser = struct {
    ...
    fn parseFunctionDef(self: *Parser) !*Node {
        ...
        return node;
            ...
            // Allow type keywords to be used as function names (e.g., str(), int())
            .Str, .I32, .I64, .F32, .F64, .Bool => {
                const func_name = tok.lexeme;
                self.advance();
                if (self.current().kind == .LeftParen) {
                    _ = try self.expect(.LeftParen);
                    const call = try ast.createFunctionCall(self.allocator, try ast.createIdentifier(self.allocator, func_name));
                    ...
                    // Used as type annotation, treat as identifier
                    node = try ast.createIdentifier(self.allocator, func_name);
                }
            },
            .LeftParen => {
                _ = try self.expect(.LeftParen);
